...
export class AddProductComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.invent.getIdentifier('fishstatus.pending').subscribe(it => {
      this.pendingStatus = it['fishstatus']['pending']['id'];
    });
  ...
  async onSubmit() {
    ...
    const data: any = {
      ...
      'mortalityRate': 1,
      'waterLostRate': '0',
      'status': this.user['role'] !== 0 ? this.pendingStatus : product.status,
