...
export class ItemsByStatusComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.getStatus();
    this.getOrders();
    this.invent.getIdentifier('orderstatus.pending_seller_confirmation.pending_fulfillment.cancelled_order').subscribe(it => {
      ...
      this.cancelStatus = it['orderstatus']['cancelled_order']['id'];
    });
  }
  ...
  cancelItem(itemID: string) {
    this.orderService.updateStatus(this.cancelStatus, itemID, this.user).subscribe(
      result => {
